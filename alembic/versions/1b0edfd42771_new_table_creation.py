"""new table creation

Revision ID: 1b0edfd42771
Revises: 
Create Date: 2025-01-01 18:01:55.997879

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '1b0edfd42771'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('jobs_v2',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('platform', sa.String(length=50), nullable=True),
    sa.Column('platform_link', sa.String(length=255), nullable=True),
    sa.Column('job_title', sa.String(length=250), nullable=True),
    sa.Column('job_link', sa.Text(), nullable=True),
    sa.Column('company', sa.String(length=120), nullable=True),
    sa.Column('company_link', sa.Text(), nullable=True),
    sa.Column('posted_on', sa.String(length=255), nullable=True),
    sa.Column('position', sa.String(length=100), nullable=True),
    sa.Column('salary', sa.String(length=50), nullable=True),
    sa.Column('duration', sa.String(length=50), nullable=True),
    sa.Column('applicant', sa.String(length=100), nullable=True),
    sa.Column('last_updated', sa.String(length=255), nullable=True),
    sa.Column('job_description', sa.Text(), nullable=True),
    sa.Column('experience_level', sa.String(length=50), nullable=True),
    sa.Column('location', sa.String(length=250), nullable=True),
    sa.Column('last_date_application', sa.String(length=250), nullable=True),
    sa.Column('opening', sa.String(length=50), nullable=True),
    sa.Column('industry', sa.String(length=150), nullable=True),
    sa.Column('job_type', sa.String(length=100), nullable=True),
    sa.Column('industry_function', sa.Text(), nullable=True),
    sa.Column('skill_list', sa.Text(), nullable=True),
    sa.Column('early_applicant', sa.String(length=50), nullable=True),
    sa.Column('job_id', sa.String(length=50), nullable=True),
    sa.Column('job_role', sa.String(length=150), nullable=True),
    sa.Column('interview_process', sa.String(length=100), nullable=True),
    sa.Column('education', sa.Text(), nullable=True),
    sa.Column('specialization', sa.String(length=200), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('jobs_v2')
    # ### end Alembic commands ###
