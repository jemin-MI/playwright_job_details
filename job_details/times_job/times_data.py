...
from contants_dir.constant import TimesJob_link, TimesJob, input_job_role, input_job_times_location
...
async def pagination(page_wise, data_list, page):
    page_wise.append({'page_' + str(0 + 1): data_list})

    pagination_div = page.locator('.srp-pagination em')
    if await pagination_div.count() > 1:
        if await pagination_div.nth(1).count() > 1:

            await pagination_div.nth(1).click()
            await main_iterator(page, page_wise)
            await page.wait_for_timeout(3000)
        else:
            logger.info("New page clicked")
            return None

    with open(f'page_wise_{TimesJob}.json', 'w') as file:
        file.write(json.dumps(page_wise))


async def main_iterator(page, page_wise):
    link_list = []
    ...
    await pagination(page_wise, data_list, page)
...
async def main():
    async with async_playwright() as p:
        ...
        await page.fill('#txtLocation', input_job_times_location)
